...
# Most important function at top of file.
def replace_bert_attn():
    # For now omitted flash attention and RoPE for BERT.
    transformers.models.bert.modeling_bert.BertAttention.forward = forward_noflashattn
...
def forward_noflashattn(
    ...
    padding_mask: Optional[torch.LongTensor] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
    ...
    if attention_mask is not None:
        if attention_mask.size() != (bsz * num_group, 1, group_size, group_size):
            raise ValueError(
                f"Attention mask should be of size {(bsz * num_group, 1, group_size, group_size)}, "
